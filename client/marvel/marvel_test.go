...
func TestStory_UnmarshalJSON(t *testing.T) {
	t.Run("Success", func(t *testing.T) {
		for _, tc := range []struct {
			...
			{
				desc: "String",
				b:    []byte(`{"id":1,"description":"foo","title":"bar"}`),
				want: Story{ID: 1, Description: "foo", Title: "bar"},
			},
			{
				desc: "Int",
				b:    []byte(`{"id":1,"description":1234,"title":5678}`),
				want: Story{ID: 1, Description: "1234", Title: "5678"},
			},
			{
				desc: "Float",
				b:    []byte(`{"id":1,"description":12.34,"title":56.78}`),
				want: Story{ID: 1, Description: "12.34", Title: "56.78"},
			},
		} {
			t.Run(tc.desc, func(t *testing.T) {
				var gotStory Story
	...
	t.Run("Error", func(t *testing.T) {
		for _, tc := range []struct {
			...
			{
				desc: "InvalidTypeDescription",
				in:   []byte(`{"id":1, "Description":false}`),
				err:  &json.UnmarshalTypeError{},
			},
			{
